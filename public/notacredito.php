...
// ✅ FUNCIÓN PARA GUARDAR METADATOS EN CSV (IGUAL QUE FACTURA)
...
// ✅ DETECTAR ESTRUCTURA DINÁMICAMENTE (MISMO QUE FACTURA)
$docData = null;
$config = null;
$firstItem = null;
...
try {
    // ✅ CONFIGURAR GREENTER CON DATOS DEL JSON (IGUAL QUE FACTURA)
    $see = new See();
    ...
    // ✅ CONSTRUIR NOTA DE CRÉDITO (CAMBIOS PRINCIPALES AQUÍ)
    $company = new Company();
    ...
    // ✅ DOCUMENTO AFECTADO (NUEVO PARA NOTA DE CRÉDITO)
    $docAfectado = new Document();
    $docAfectado->setTipoDoc($docData['tipDocAfectado'])
                ->setNroDoc($docData['numDocfectado']);
    ...
    // ✅ CONSTRUIR NOTA DE CRÉDITO (CLASE NOTE EN LUGAR DE INVOICE)
    $note = new Note();
    $note->setUblVersion($docData['ublVersion'])
         ->setTipoDoc($docData['tipoDoc'])  // "07" para nota de crédito
         ->setSerie($docData['serie'])
         ->setCorrelativo($docData['correlativo'])
         ->setFechaEmision(new DateTime($docData['fechaEmision']))
         ->setTipDocAfectado($docData['tipDocAfectado'])    // "01" factura
         ->setNumDocfectado($docData['numDocfectado'])      // "F001-111"
         ->setCodMotivo($docData['codMotivo'])              // "07"
         ->setDesMotivo($docData['desMotivo'])              // "DEVOLUCION POR ITEM"
    ...
    // ✅ GUIAS (OPCIONAL - SI VIENEN EN EL JSON)
    if (isset($docData['guias']) && is_array($docData['guias'])) {
        $guias = [];
        foreach ($docData['guias'] as $guiaData) {
            $guia = new Document();
            $guia->setTipoDoc($guiaData['tipoDoc'])
                 ->setNroDoc($guiaData['nroDoc']);
            $guias[] = $guia;
        }
        $note->setGuias($guias);
    }
    ...
    // ✅ OBTENER XML FIRMADO
    $xml_firmado = $see->getXmlSigned($note);
